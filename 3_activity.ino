...
void isRunning() {
  ...
  if ((AcZ > -15000 and  sumAcXY < -10000 and sumAcXY > lastsumAcXY) or isrunstep == 1 ) {   //if new step is detected to start, or if step is expected. AcZ value so it doesnt trigger when face up
    isrunstep = 1;
    ...
    if (stepDuration < maxStepTime) {                       //if max time per step hasnt elapsed
      if ((AcNet > AcMax and AcNet < AcNetMax) and stepDuration > minStepTime ) { //if between max and min step times, gynet exceeds gymax, increment steps
        if (AcNetMax < 20000)
...
void isWalking() {
  if (isrunstep == 0 and ((AcZ > -15000 and  GyNet < GyMin) or isstep == 1 )) {   //if new step is detected to start, or if step is expected. AcZ value so it doesnt trigger when face up
    isstep = 1;
